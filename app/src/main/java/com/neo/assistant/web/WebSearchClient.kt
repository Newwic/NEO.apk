...
class WebSearchClient {
    ...
    private suspend fun searchCurrency(query: String): List<Result> = withContext(Dispatchers.IO) {
        try {
            ...
            client.newCall(req).execute().use { res ->
                ...
                listOf(Result("$base/THB", "1 $base ≈ ${String.format("%.2f", rate)} บาท", url))
            }
    ...
    private fun decodeXml(s: String): String = s.replace("<![CDATA[", "").replace("]]>", "")
    ...
    private suspend fun searchDuckDuckGo(query: String): List<Result> = withContext(Dispatchers.IO) {
        try {
            ...
            val req = Request.Builder().url("https://api.duckduckgo.com/?q=$q&format=json&no_html=1&skip_disambig=1").header("User-Agent", "NEO-Android/1.0").build()
            client.newCall(req).execute().use { res ->
                if (!res.isSuccessful) return@withContext emptyList()
                val json = JSONObject(res.body?.string().orEmpty())
                val out = mutableListOf<Result>()
                val abstract = json.optString("AbstractText"); val abstractUrl = json.optString("AbstractURL")
                if (abstract.isNotBlank() && abstractUrl.isNotBlank()) out += Result(json.optString("Heading", "Web"), abstract, abstractUrl)
                collectTopics(json.optJSONArray("RelatedTopics"), out); out.take(3)
            }
        } catch (_: Exception) { emptyList() }
    }
    ...
    private fun collectTopics(array: JSONArray?, out: MutableList<Result>) {
        ...
        for (i in 0 until array.length()) {
            val item = array.optJSONObject(i) ?: continue; val nested = item.optJSONArray("Topics")
            if (nested != null) collectTopics(nested, out) else {
                val text = item.optString("Text"); val url = item.optString("FirstURL")
    ...
    private suspend fun searchThaiWikipedia(query: String): List<Result> = withContext(Dispatchers.IO) {
        try {
            val q = URLEncoder.encode(query, "UTF-8")
            val req = Request.Builder().url("https://th.wikipedia.org/w/api.php?action=query&list=search&srsearch=$q&format=json&utf8=1&srlimit=3").header("User-Agent", "NEO-Android/1.0").build()
            client.newCall(req).execute().use { res ->
                if (!res.isSuccessful) return@withContext emptyList()
